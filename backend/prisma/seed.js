...
async function seedCountries() {
    ...
    try {
        ...
        console.log(`✅ Countries seeded: ${createdCount} created, ${updatedCount} updated`);
    }
    catch (error) {
...
async function seedRoleCatalog() {
    ...
    try {
        ...
        console.log(`✅ Role catalog seeded: ${createdCount} created, ${updatedCount} updated`);
    }
    catch (error) {
...
async function seedQualifications() {
    ...
    try {
        const qualificationsPath = path.join(__dirname, 'seeds', 'qualifications.json');
        const qualificationsData = JSON.parse(fs.readFileSync(qualificationsPath, 'utf8'));
        let createdCount = 0;
        let updatedCount = 0;
        ...
        console.log(`✅ Qualifications seeded: ${createdCount} created, ${updatedCount} updated`);
    }
    catch (error) {
...
async function seedQualificationAliases() {
    ...
    try {
        const aliasesPath = path.join(__dirname, 'seeds', 'qualification-aliases.json');
            ...
            if (qualification) {
                try {
                    ...
                    if (!error.message.includes('Unique constraint')) {
                        console.warn(`Warning: Could not create alias "${aliasData.alias}":`, error.message);
                    }
                }
            }
            else {
                console.warn(`Warning: Qualification not found for shortName: ${aliasData.qualificationShortName}`);
            }
        }
...
async function seedRoleRecommendedQualifications() {
    ...
    try {
        const recommendationsPath = path.join(__dirname, 'seeds', 'role-recommended-qualifications.json');
        ...
        for (const recommendation of recommendationsData) {
            ...
            if (role && qualification) {
                try {
                    ...
                    if (!error.message.includes('Unique constraint')) {
                        console.warn(`Warning: Could not create recommendation:`, error.message);
                    }
                }
            }
            else {
                console.warn(`Warning: Role or qualification not found for: ${recommendation.roleSlug} -> ${recommendation.qualificationShortName}`);
            }
        }
        console.log(`✅ Role recommended qualifications seeded: ${createdCount} created`);
    }
    catch (error) {
...
async function main() {
    ...
    console.log(`🔍 Recruiter 1: +919876543215 / recruiter123 (recruiter1@affiniks.com)`);
    console.log(`🔍 Recruiter 2: +919876543216 / recruiter123 (recruiter2@affiniks.com)`);
    console.log(`🔍 Recruiter 3: +919876543217 / recruiter123 (recruiter3@affiniks.com)`);
    console.log(`🔍 Recruiter 4: +919876543218 / recruiter123 (recruiter4@affiniks.com)`);
    console.log(`🔍 Recruiter 5: +919876543219 / recruiter123 (recruiter5@affiniks.com)`);
    ...
    console.log('👥 Only eligible roles (Team Lead, Recruiter, Documentation Executive, Processing Executive) are assigned to the Default Team!');
    console.log('🔒 Higher roles (CEO, Director, Manager, Team Head, System Admin) are not team members and can manage teams!');
}
main()
    .catch((e) => {
