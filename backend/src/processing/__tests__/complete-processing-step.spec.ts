...
describe('ProcessingService - completeProcessingStep (medical & emigration)', () => {
  ...
  it('throws when emigrationStatus is not provided for emigration step', async () => {
    jest.spyOn(prisma.processingStep, 'findUnique' as any).mockResolvedValue({
      id: 'step-2',
      status: 'in_progress',
      template: { key: 'emigration', label: 'Emigration' },
      processingCandidateId: 'pc-2',
    });

    await expect(service.completeProcessingStep('step-2', 'user-2', {} as any)).rejects.toThrow();
  });

  it('persists emigrationStatus and completes when provided', async () => {
    const step = {
      id: 'step-2',
      status: 'in_progress',
      template: { key: 'emigration', label: 'Emigration' },
      processingCandidateId: 'pc-2',
      processingCandidate: { candidateId: 'cand-2', projectId: 'proj-2', roleNeededId: 'role-2' },
    };
    ...
    const txMock = {
      processingStep: { update: jest.fn().mockResolvedValue(undefined), findFirst: jest.fn().mockResolvedValue(null) },
      processingHistory: { create: jest.fn().mockResolvedValue(undefined) },
      candidateProjects: { findFirst: jest.fn().mockResolvedValue({ recruiterId: 'recr-2' }) },
      processingCandidate: { update: jest.fn().mockResolvedValue(undefined) },
    };

    jest.spyOn(prisma, '$transaction' as any).mockImplementation(async (cb: any) => cb(txMock));

    await service.completeProcessingStep('step-2', 'user-2', { emigrationStatus: 'COMPLETED', notes: 'Emigration cleared' } as any);

    expect(txMock.processingStep.update).toHaveBeenCalledWith({
      where: { id: 'step-2' },
      data: expect.objectContaining({ status: 'completed', emigrationStatus: 'COMPLETED' }),
    });
    ...
    expect(txMock.processingHistory.create).toHaveBeenCalledWith({
      data: expect.objectContaining({
        ...
        step: 'emigration',
        changedById: 'user-2',
        notes: expect.stringContaining('Emigration cleared'),
      }),
    });
  });

  it('persists emigrationStatus then cancels processing when emigration failed', async () => {
    const step = {
      id: 'step-3',
      status: 'in_progress',
      template: { key: 'emigration', label: 'Emigration' },
      processingCandidateId: 'pc-3',
    };
    ...
    const res = await service.completeProcessingStep('step-3', 'user-3', { emigrationStatus: 'FAILED', notes: 'Visa rejected' } as any);

    expect(updateSpy).toHaveBeenCalledWith({ where: { id: 'step-3' }, data: { emigrationStatus: 'FAILED' } });
    expect(cancelSpy).toHaveBeenCalledWith('step-3', 'user-3', expect.stringContaining('Visa rejected'));
    expect(res).toEqual(expect.objectContaining({ success: true }));
  });
});
