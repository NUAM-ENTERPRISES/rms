...
describe('ProcessingService - getMedicalRequirements', () => {
  ...
  it('returns HRD-shaped payload for medical step', async () => {
    const pcId = 'pc-1';
    const template = { id: 'tpl-med', key: 'medical' };
    ...
    jest.spyOn(prisma.processingStep, 'findFirst' as any).mockResolvedValue({
      ...
      status: 'in_progress',
      documents: [
        { id: 'psd-1', candidateProjectDocumentVerification: { id: 'ver-1', status: 'verified', document: { docType: 'medical_fitness' }, createdAt: new Date(), updatedAt: new Date() }, uploadedBy: 'u1', status: 'verified', createdAt: new Date(), updatedAt: new Date() },
      ],
      template,
    });
    ...
    const res = await service.getMedicalRequirements(pcId);
    ...
    expect(res.requiredDocuments).toEqual(expect.arrayContaining([expect.objectContaining({ docType: 'medical_fitness' })]));
    expect(res.processing_documents.length).toBeGreaterThanOrEqual(1);
    // step-specific flag present
    expect(res).toHaveProperty('isMedicalCompleted');
  });
});
