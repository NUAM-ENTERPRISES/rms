...
export class ScreeningTemplatesService {
  ...
  async create(dto: CreateScreeningTemplateDto) {
    ...
    return this.prisma.$transaction(async (tx) => {
      ...
      if (dto.items && dto.items.length > 0) {
        await tx.screeningTemplateItem.createMany({
      ...
      return tx.screeningTemplate.findUnique({
   ...
   */
  async update(id: string, dto: UpdateScreeningTemplateDto) {
  ...
  async remove(id: string) {
    ...
    await this.prisma.screeningTemplate.delete({
      where: { id },
    });
  ...
  async addItem(templateId: string, dto: CreateTemplateItemDto) {
    ...
    const existingItem = await this.prisma.screeningTemplateItem.findUnique({
      where: {
    ...
    return this.prisma.screeningTemplateItem.create({
      data: {
        templateId,
        category: dto.category,
        criterion: dto.criterion,
  ...
  async updateItem(
    ...
    const item = await this.prisma.screeningTemplateItem.findFirst({
      where: {
        id: itemId,
        templateId,
      },
    ...
    if (dto.criterion || dto.category) {
      ...
      const existing = await this.prisma.screeningTemplateItem.findFirst({
        where: {
          templateId,
    ...
    return this.prisma.screeningTemplateItem.update({
      where: { id: itemId },
      data: {
        category: dto.category ?? item.category,
        criterion: dto.criterion ?? item.criterion,
  ...
  async removeItem(templateId: string, itemId: string) {
    ...
    await this.prisma.screeningTemplateItem.delete({
      where: { id: itemId },
    });
