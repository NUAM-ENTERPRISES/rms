...
export class NotificationsProcessor extends WorkerHost {
  ...
  async handleCandidateApprovedForClientInterview(
    ...
    try {
      ...
      // Notify only the assigned Documentation Executive (send-for-verification RR).
      const assignedDocumentationExecutiveId =
        candidateProjectMap.assignedDocumentationExecutiveId;
      if (assignedDocumentationExecutiveId) {
        const idemKeyExec = `${eventId}:${assignedDocumentationExecutiveId}:screening_passed_exec`;

        await this.notificationsService.createNotification({
          userId: assignedDocumentationExecutiveId,
          type: 'screening_passed',
