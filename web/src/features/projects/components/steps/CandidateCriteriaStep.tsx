...
export const CandidateCriteriaStep: React.FC<CandidateCriteriaStepProps> = ({
  ...
  return (
    <div className="space-y-6">
      {/* ───── BULK UPDATE TOOL ───── */}
     <Card className="relative border-0 shadow-md bg-gradient-to-r from-emerald-50 via-white to-teal-50/50 dark:bg-black dark:from-black dark:via-black dark:to-black overflow-hidden">
  <CardHeader className="pb-1 pt-4 px-5 relative border-b border-slate-200 dark:border-slate-800">
    <div className="flex items-center gap-2.5">
      <div className="p-1.5 rounded-lg bg-emerald-100 dark:bg-emerald-950/50 border border-emerald-200 dark:border-emerald-800/50">
  ...
  <CardContent className="relative px-5 pb-5 pt-3 bg-white dark:bg-black">
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 items-end">
      ...
      <div className="space-y-1.5">
        <Label className="text-[11px] font-semibold text-slate-600 dark:text-slate-300 flex items-center gap-1">
          <User className="h-3 w-3 text-slate-500 dark:text-slate-400" /> Experience (Min-Max)
        </Label>
        <div className="flex gap-1.5">
          <Input
            ...
            className="bg-white dark:bg-slate-900 border-slate-200 dark:border-slate-700 h-8 rounded-lg text-xs text-slate-900 dark:text-slate-100 focus:border-emerald-500 dark:focus:border-emerald-500 focus:ring-emerald-500/20 dark:focus:ring-emerald-500/20"
          />
          <Input
            type="number"
            placeholder="Max"
      ...
      <div className="space-y-1.5">
        <Label className="text-[11px] font-semibold text-slate-600 dark:text-slate-300 flex items-center gap-1">
          <Clock className="h-3 w-3 text-slate-500 dark:text-slate-400" /> Shift & Gender
        </Label>
        <div className="flex gap-1.5">
          <Select
          ...
          >
            <SelectTrigger className="bg-white dark:bg-slate-900 border-slate-200 dark:border-slate-700 h-8 rounded-lg text-xs text-slate-900 dark:text-slate-100 focus:border-emerald-500 dark:focus:border-emerald-500 focus:ring-emerald-500/20 dark:focus:ring-emerald-500/20 flex-1">
      ...
      <div className="space-y-1.5">
        <Label className="text-[11px] font-semibold text-slate-600 dark:text-slate-300 flex items-center gap-1">
          <Award className="h-3 w-3 text-slate-500 dark:text-slate-400" /> Salary Range
        </Label>
        <div className="flex gap-1.5">
          <Input
            placeholder="Min"
          ...
          >
            <SelectTrigger className="bg-white dark:bg-slate-900 border-slate-200 dark:border-slate-700 h-8 px-1.5 rounded-lg text-[10px] w-16 text-slate-900 dark:text-slate-100 focus:border-emerald-500 dark:focus:border-emerald-500">
              <SelectValue />
            </SelectTrigger>
            <SelectContent className="bg-white dark:bg-slate-900 border-slate-200 dark:border-slate-700 text-slate-900 dark:text-slate-100">
      ...
      <div className="space-y-1.5 lg:col-span-1 xl:col-span-1">
        <Label className="text-[11px] font-semibold text-slate-600 dark:text-slate-300 flex items-center gap-1">
          <MapPin className="h-3 w-3 text-slate-500 dark:text-slate-400" /> Location & Religion
        </Label>
        <div className="flex gap-1.5">
          <Popover>
            <PopoverTrigger asChild>
              <Button 
                variant="outline" 
                className="h-8 text-[10px] flex-1 justify-between bg-white dark:bg-slate-900 border-slate-200 dark:border-slate-700 hover:bg-slate-50 dark:hover:bg-slate-800"
              >
                <span className="truncate">
                  {bulkCriteria.candidateStates.length === 0 ? "Any State" : `${bulkCriteria.candidateStates.length} States`}
                </span>
                <ChevronDown className="h-3 w-3 opacity-50 text-slate-400 dark:text-slate-500" />
              </Button>
            </PopoverTrigger>
            <PopoverContent className="w-64 p-2 bg-white dark:bg-slate-900 border-slate-200 dark:border-slate-800">
              <div className="max-h-48 overflow-y-auto space-y-1 scrollbar-thin scrollbar-thumb-slate-200 dark:scrollbar-thumb-slate-700">
                <div 
                  className="flex items-center gap-2 p-1.5 hover:bg-slate-50 dark:hover:bg-slate-800 rounded-md cursor-pointer"
                  onClick={() => setBulkCriteria(p => ({...p, candidateStates: []}))}
                >
                  <Checkbox checked={bulkCriteria.candidateStates.length === 0} className="border-slate-300 dark:border-slate-600" />
                  <span className="text-xs text-slate-700 dark:text-slate-300">All India</span>
                </div>
                {indianStates.map(state => (
                  ...
                  >
                    <Checkbox 
                      checked={bulkCriteria.candidateStates.includes(state.id)} 
                      className="border-slate-300 dark:border-slate-600 data-[state=checked]:bg-emerald-600 dark:data-[state=checked]:bg-emerald-600"
                    />
                    <span className="text-xs text-slate-700 dark:text-slate-300">{state.name}</span>
                  </div>
                ))}
              </div>
            </PopoverContent>
          ...
          <Popover>
            <PopoverTrigger asChild>
              ...
              >
                <span className="truncate">
                  {bulkCriteria.candidateReligions.length === 0 ? "Any Religion" : `${bulkCriteria.candidateReligions.length} Rel.`}
                </span>
                <ChevronDown className="h-3 w-3 opacity-50 text-slate-400 dark:text-slate-500" />
              </Button>
            </PopoverTrigger>
            <PopoverContent className="w-48 p-2 bg-white dark:bg-slate-900 border-slate-200 dark:border-slate-800">
              <div className="space-y-1">
                {religions.map(r => (
                  ...
                  >
                    <Checkbox 
                      checked={bulkCriteria.candidateReligions.includes(r.id)} 
                      className="border-slate-300 dark:border-slate-600 data-[state=checked]:bg-emerald-600 dark:data-[state=checked]:bg-emerald-600"
                    />
                    <span className="text-xs text-slate-700 dark:text-slate-300">{r.name}</span>
                  </div>
                ))}
              </div>
            </PopoverContent>
      ...
      <div className="space-y-1.5 lg:col-span-1 xl:col-span-1">
        <Label className="text-[11px] font-semibold text-slate-600 dark:text-slate-300 flex items-center gap-1">
          <BookOpen className="h-3 w-3 text-slate-500 dark:text-slate-400" /> Education Requirements
        </Label>
        <ProjectQualificationSelect
      ...
      <div className="space-y-1.5 lg:col-span-1 xl:col-span-1">
        <Label className="text-[11px] font-semibold text-slate-600 dark:text-slate-300 flex items-center gap-1">
          <Award className="h-3 w-3 text-slate-500 dark:text-slate-400" /> Common Skills
        </Label>
        <div className="flex gap-1">
          <Input
            ...
            className="bg-white dark:bg-slate-900 border-slate-200 dark:border-slate-700 h-8 rounded-lg text-xs text-slate-900 dark:text-slate-100 focus:border-emerald-500 dark:focus:border-emerald-500"
          />
          <Button 
            size="sm" 
            className="h-8 w-8 p-0 bg-emerald-600 hover:bg-emerald-700 dark:bg-emerald-700 dark:hover:bg-emerald-600"
            onClick={() => {
              if (bulkSkillInput.trim()) {
                setBulkCriteria(p => ({...p, requiredSkills: [...p.requiredSkills, bulkSkillInput.trim()]}));
                setBulkSkillInput("");
          ...
          <div className="flex flex-wrap gap-1 mt-1">
            {bulkCriteria.requiredSkills.map((s, i) => (
              <Badge 
                key={i} 
                variant="secondary" 
                className="px-1 py-0 text-[9px] bg-emerald-50 dark:bg-emerald-950/50 text-emerald-700 dark:text-emerald-300 border border-emerald-100 dark:border-emerald-800/50"
              >
                {s}
                <X 
                  className="h-2 w-2 ml-1 cursor-pointer text-emerald-600 dark:text-emerald-400" 
                  onClick={() => setBulkCriteria(p => ({...p, requiredSkills: p.requiredSkills.filter((_, idx) => idx !== i)}))} 
                />
              </Badge>
            ))}
          </div>
      ...
      {/* Checkboxes & Age */}
      <div className="col-span-1 md:col-span-2 lg:col-span-3 xl:col-span-4 flex flex-wrap items-center gap-x-6 gap-y-3 pt-2 border-t border-emerald-100 dark:border-emerald-900/50">
        <div className="flex items-center gap-4">
          <div className="flex items-center gap-2 cursor-pointer" onClick={() => setBulkCriteria(p => ({...p, accommodation: !p.accommodation}))}>
            <Checkbox 
              checked={bulkCriteria.accommodation} 
              className="border-slate-300 dark:border-slate-600 data-[state=checked]:bg-emerald-600 dark:data-[state=checked]:bg-emerald-600"
            />
            <span className="text-[11px] font-medium text-slate-700 dark:text-slate-300">Accommodation</span>
          </div>
          <div className="flex items-center gap-2 cursor-pointer" onClick={() => setBulkCriteria(p => ({...p, food: !p.food}))}>
            <Checkbox 
              checked={bulkCriteria.food} 
              className="border-slate-300 dark:border-slate-600 data-[state=checked]:bg-emerald-600 dark:data-[state=checked]:bg-emerald-600"
            />
            <span className="text-[11px] font-medium text-slate-700 dark:text-slate-300">Food</span>
          </div>
          <div className="flex items-center gap-2 cursor-pointer" onClick={() => setBulkCriteria(p => ({...p, transport: !p.transport}))}>
            <Checkbox 
              checked={bulkCriteria.transport} 
              className="border-slate-300 dark:border-slate-600 data-[state=checked]:bg-emerald-600 dark:data-[state=checked]:bg-emerald-600"
            />
            <span className="text-[11px] font-medium text-slate-700 dark:text-slate-300">Transport</span>
          </div>
        </div>

        <div className="flex-1 flex items-center gap-3">
          <div className="flex-1 max-w-[200px] flex items-center gap-2">
            <Label className="text-[11px] font-semibold text-slate-600 dark:text-slate-300 whitespace-nowrap">Age Req.</Label>
            <Input
          ...
          <div className="flex-1 flex items-center gap-2">
            <Label className="text-[11px] font-semibold text-slate-600 dark:text-slate-300 whitespace-nowrap">Certs.</Label>
            <Input
        ...
        <Button
          ...
          className="h-9 px-6 bg-emerald-600 hover:bg-emerald-700 dark:bg-emerald-700 dark:hover:bg-emerald-600 text-white rounded-lg text-xs font-bold gap-2 shadow-lg shadow-emerald-100 dark:shadow-emerald-900/30 active:scale-[0.98] transition-all ml-auto"
        >
     ...
     <div className="space-y-3 dark:bg-black dark:text-white p-2 rounded-xl">
  <div className="flex items-center justify-between">
    <div className="flex items-center gap-3">
      <Briefcase className="h-4 w-4 text-emerald-500" />
      <h3 className="text-sm font-bold text-slate-800 dark:text-white">
        Role Criteria
        <span className="ml-2 text-xs font-normal text-slate-500 dark:text-gray-300">
          ...
          >
            <Card
              className={cn(
                ...
                `border-slate-200 dark:border-slate-700 ${cardBgColor} dark:bg-slate-900 hover:border-emerald-300 hover:ring-1 hover:ring-emerald-100`,
                displayError &&
                  "border-destructive ring-1 ring-destructive/20 hover:border-destructive hover:ring-destructive/30"
              )}
            >
              <div
                className={cn(
              ...
              <div className="absolute top-1.5 right-1.5 p-1 rounded-md bg-white/80 dark:bg-slate-800 text-slate-400 dark:text-gray-300 border border-transparent group-hover:border-emerald-200 group-hover:text-emerald-600 transition-all opacity-0 group-hover:opacity-100 z-10">
              ...
              <CardContent className="px-3 pb-3 pt-2.5 space-y-2">
                <div className="flex items-start gap-2">
                  <span className="inline-flex items-center justify-center w-5 h-5 rounded-full text-[9px] font-bold flex-shrink-0 bg-emerald-100 dark:bg-emerald-900 text-emerald-600 dark:text-emerald-300">
                    {index + 1}
                  </span>
                  <div className="min-w-0 flex-1">
                    <h4 className="text-[11px] font-bold text-slate-800 dark:text-white truncate leading-tight">
                      {role.designation || `Role ${index + 1}`}
                    </h4>
                    <p className="text-[9px] text-slate-400 dark:text-gray-400 truncate">
                      Qty: {role.quantity || 1}
                    </p>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-2 gap-1.5 text-[10px]">
                  <div className="flex items-center gap-1 bg-white/60 dark:bg-slate-800 rounded px-1.5 py-1">
                    <Briefcase className="h-3 w-3 text-green-500 flex-shrink-0" />
                    <span className="text-slate-600 dark:text-gray-300 truncate">
                      {hasExperience
                  ...
                  <div className="flex items-center gap-1 bg-white/60 dark:bg-slate-800 rounded px-1.5 py-1">
                    ...
                    <span className="text-slate-600 dark:text-gray-300 truncate capitalize">
                ...
                {(schemaError || hasLocalError) && (
                  <div className="bg-red-50 dark:bg-red-900/20 p-1.5 rounded-md border border-red-100 dark:border-red-800 flex items-start gap-1">
                    <AlertCircle className="h-2.5 w-2.5 text-red-500 mt-0.5 flex-shrink-0" />
                    <p className="text-[9px] text-red-600 dark:text-red-400 font-bold leading-tight">
                      {schemaError
