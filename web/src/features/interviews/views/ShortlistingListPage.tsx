...
import { useGetShortlistPendingQuery, useUpdateClientDecisionMutation, useUpdateBulkClientDecisionMutation } from "../api";
...
export default function ShortlistingListPage() {
  ...
  const { data, isLoading, isError, refetch } = useGetShortlistPendingQuery({
    page: 1,
    limit: 10,
  ...
  // Get selected candidates for bulk modal
  ...
  if (isError) {
    return (
      <div className="h-screen flex flex-col items-center justify-center gap-4">
        <p className="text-destructive font-medium">Failed to load pending shortlists</p>
          ...
          <ScrollArea className="flex-1">
            {displayed.length === 0 ? (
              <div className="p-8 text-center text-muted-foreground">
                ...
                <p className="font-medium">No items pending shortlist</p>
                <p className="text-xs mt-1">Candidates in 'Submitted to Client' status will appear here</p>
              </div>
            ) : (
          ...
          {selected ? (
            <ScrollArea className="h-full">
              <div className="p-5 max-w-4xl mx-auto space-y-5">
                <div className="flex items-start justify-between">
                  <div>
                    <h2 className="text-2xl font-bold bg-clip-text text-foreground">Submission Details</h2>
                    <p className="text-sm text-muted-foreground mt-1">
                      {selected.updatedAt ? format(new Date(selected.updatedAt), "MMMM d, yyyy • h:mm a") : "Sent to client"}
                    </p>
                  </div>

                  <div className="flex items-center gap-3">
                    <Badge className={cn("px-3 py-1 text-sm font-medium shadow-sm", "bg-emerald-100 text-emerald-800")}>
                      {selected.subStatus?.label || "Sent"}
                    </Badge>

                    <Button 
                      size="sm" 
                      className="bg-amber-600 hover:bg-amber-700 text-white shadow-md transition-all active:scale-95" 
            ...
            <div className="h-full flex items-center justify-center text-muted-foreground">
              <div className="text-center">
                ...
                <p className="font-medium">No candidates pending shortlist</p>
