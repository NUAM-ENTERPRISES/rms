...
interface HRDReminderModalProps {
  isOpen: boolean;
  onClose: () => void;
  reminder: HRDReminder | null;
}

export function HRDReminderModal({
  isOpen,
  onClose,
  reminder,
}: HRDReminderModalProps) {
  ...
  // Defensive access: prefer the processingStep.processingCandidate (present in API) then fall back to processing
  ...
  // Some project shapes use `title` and others use `name` — cast to `any` to safely access either property
  const projectName = (project as any)?.title || (project as any)?.name || "Project";

  // Resolve processingId and processingCandidateId reliably from available fields
  ...
  // Candidate contact details (defensive)
  ...
  // SubmittedAt formatting (prefer step submittedAt, else fallback to reminder.createdAt)
  ...
  // Format time since submission (or since step creation)
  ...
  const handleViewProcessing = () => {
    // Navigate to processing candidate details page using processingCandidateId
        ...
        <div className="space-y-2 py-3">
          ...
          <div className="bg-slate-100 rounded-xl p-3 border border-slate-300">
            <p className="text-sm text-slate-700 leading-relaxed">
              💡 <span className="font-semibold">Action Required:</span> Please review and verify hard HRD documents or update the status to avoid delays.
            </p>
          </div>
        </div>
