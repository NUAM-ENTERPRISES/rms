...
import { WORKFLOW_ACCORDION_OPEN_PURPLE, SURFACE_EMERALD_SOFT, SURFACE_RED_SOFT, SURFACE_BLUE_SOFT } from "@/lib/page-shell-styles";
...
export default function CandidateInterviewWorkflowPage() {
  ...
  if (error || !candidate) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[400px] space-y-4">
        ...
        <h2 className="text-xl font-semibold text-foreground">Error loading interview details</h2>
  ...
  const getOutcomeBadge = (outcome: string) => {
    switch (outcome?.toLowerCase()) {
      ...
      case "approved":
      case "selected":
        return <Badge className={cn(SURFACE_EMERALD_SOFT, "border font-semibold text-[10px] gap-1")}><UserCheck className="h-3 w-3" /> Passed</Badge>;
      case "failed":
      case "rejected":
        return <Badge className={cn(SURFACE_RED_SOFT, "border font-semibold text-[10px] gap-1")}><XCircle className="h-3 w-3" /> Failed</Badge>;
      case "pending":
      case "scheduled":
      default:
        return <Badge className={cn(SURFACE_BLUE_SOFT, "border font-semibold text-[10px] gap-1")}><Clock className="h-3 w-3" /> {outcome || 'Scheduled'}</Badge>;
    }
  };
      ...
      <div className="flex flex-col sm:flex-row gap-3 items-stretch sm:items-center bg-card p-3 rounded-xl shadow-sm border border-border">
        <div className="relative flex-1">
          ...
          <Input
            ...
            className="pl-10 h-10 border-border focus-visible:ring-purple-500 rounded-lg bg-muted"
            ...
            {projects.map((p: any) => (
              <AccordionItem key={p.id} value={`project-${p.id}`} className="border-none">
                <Card className="overflow-hidden border-border shadow-sm hover:shadow-md transition-all rounded-xl bg-card">
                  <div className={`flex items-center gap-2 px-5 py-4 ${WORKFLOW_ACCORDION_OPEN_PURPLE}`}>
                          ...
                          <Table>
                            <TableHeader className="bg-muted">
                              <TableRow className="hover:bg-transparent border-b border-border">
                                <TableHead className="text-[11px] font-semibold text-muted-foreground uppercase px-4 h-9 w-[25%]">Details</TableHead>
                                <TableHead className="text-[11px] font-semibold text-muted-foreground uppercase h-9 w-[15%]">Badge</TableHead>
                                <TableHead className="text-[11px] font-semibold text-muted-foreground uppercase h-9 w-[15%]">Status</TableHead>
                                <TableHead className="text-[11px] font-semibold text-muted-foreground uppercase h-9 w-[15%]">Scheduled By</TableHead>
                                <TableHead className="text-[11px] font-semibold text-muted-foreground uppercase h-9 w-[15%]">Mode</TableHead>
                                <TableHead className="text-[11px] font-semibold text-muted-foreground uppercase h-9 w-[15%]">Location/Link</TableHead>
                                <TableHead className="text-[11px] font-semibold text-muted-foreground uppercase text-right px-4 h-9 w-[15%]">Scheduled At</TableHead>
                              </TableRow>
                            </TableHeader>
                            <TableBody>
                              {p.interviews?.length > 0 ? (
                                p.interviews.map((i: any, iIndex: number) => (
                                  <TableRow key={i.id} className={`hover:bg-purple-50/40 transition-colors ${iIndex % 2 === 0 ? 'bg-card' : 'bg-muted/40'}`}>
                                    ...
                                    <TableCell>
                                      {i.scheduledBy ? (
                                        <div className="flex items-center gap-1.5">
                                          <div className="h-5 w-5 rounded-full bg-muted flex items-center justify-center text-[8px] font-bold text-muted-foreground border border-border overflow-hidden shrink-0">
                                    ...
                                    <TableCell>
                                      <span className="text-xs text-muted-foreground capitalize">{i.mode || "Online"}</span>
                                    </TableCell>
                                    <TableCell>
                                      {i.meetingLink ? (
