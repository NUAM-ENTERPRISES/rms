...
function ContributionTooltip({ active, payload }: ChartTooltipProps) {
  ...
  if (!row) return null;
  return (
    <div className="rounded-xl border border-border/80 bg-card/95 backdrop-blur-sm px-4 py-3 shadow-lg text-sm min-w-[200px]">
    ...
    >
      <div className="lg:col-span-2 space-y-3">
        <div className="grid grid-cols-2 gap-3">
          <div className="h-28 rounded-2xl bg-muted/90" />
          <div className="h-28 rounded-2xl bg-muted/90" />
        </div>
        <div className="h-[280px] rounded-2xl bg-muted/80" />
      </div>
      <div className="lg:col-span-3 h-[420px] rounded-2xl bg-muted/80" />
    </div>
  );
}
  ...
  return (
    <button
      ...
      className={cn(
        ...
        isActive
          ? "border-indigo-300 bg-card shadow-md ring-2 ring-indigo-200/80"
          : "border-border/90 bg-muted/60 hover:border-border hover:bg-muted hover:shadow-sm",
      )}
    ...
    >
      <div
        className={cn(
          ...
          isActive ? "bg-indigo-500" : "bg-muted group-hover:bg-slate-300",
        )}
        aria-hidden
      />
      <p className="pl-2 text-[10px] font-bold uppercase tracking-[0.18em] text-muted-foreground">
        {label}
      </p>
      <div className="pl-2 mt-2 flex items-end justify-between gap-2">
        <div>
          ...
          <p className="text-[10px] font-medium text-muted-foreground mt-1">points</p>
        </div>
      ...
      <div className="pl-2 mt-2.5 flex flex-wrap items-center gap-1.5">
        ...
        <span className="text-[10px] text-muted-foreground">{info.scoreRange}</span>
      </div>
      <div className="pl-2 mt-2.5 h-1.5 overflow-hidden rounded-full bg-muted">
        <div
          className={cn(
...
function TierProgressCard({
  ...
  return (
    <div className="w-full rounded-2xl border border-border/80 bg-card/80 px-4 py-3.5 text-left shadow-sm space-y-2.5">
      ...
      <p className="text-xs text-muted-foreground">
        <span className="font-semibold text-foreground">{rating}</span> requires{" "}
        {info.scoreRange}
      </p>
      {info.nextStep ? (
...
function OverallRatingHero({
  ...
  return (
    <div className="flex h-full min-h-[380px] flex-col gap-4">
      ...
      <div
        className={cn(
          "relative flex flex-1 overflow-hidden rounded-2xl border-2 bg-gradient-to-br from-card via-slate-50/30 to-indigo-50/20 shadow-md",
          RATING_CARD_BORDER[rating] ?? "border-border",
        )}
      >
        <div
        ...
        <div className="relative flex h-full w-full flex-col items-center justify-center gap-4 p-5 text-center">
          ...
          <div className="relative">
            <div
              className={cn(
                "flex h-28 w-28 items-center justify-center rounded-full border-4 bg-card shadow-inner",
                RATING_CARD_BORDER[rating] ?? "border-border",
              )}
            >
              <div>
          ...
          <div className="rounded-2xl border border-amber-200/70 bg-gradient-to-b from-amber-50/90 to-card px-5 py-3 shadow-sm">
            <RecruiterPerformanceRatingStars
              rating={rating}
              size="lg"
              variant="dashboard"
  ...
  return (
    <div className="mt-4 grid grid-cols-2 gap-2 sm:grid-cols-3">
      {rows.map((row, idx) => (
        <div
          key={row.key}
          className="rounded-xl border border-border bg-muted/50 px-3 py-2.5 transition-colors hover:border-border hover:bg-muted"
        >
...
function ContributionChart({
  ...
  if (isEmpty) {
    return (
      <div className="flex flex-1 flex-col items-center justify-center gap-3 rounded-2xl border border-dashed border-border bg-muted/40 py-12 text-slate-400">
...
export default function RecruiterPerformanceRatingSection() {
  ...
  return (
    <section
      className="relative overflow-hidden rounded-2xl border border-border/80 bg-gradient-to-b from-card via-slate-50/30 to-card shadow-sm ring-1 ring-border/60"
      ...
      <div className="border-b border-border px-5 pb-4 pt-5">
        <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
          ...
          <div className="flex flex-wrap items-center gap-2 lg:justify-end">
            <div
              className="flex rounded-xl border border-border bg-card p-0.5 shadow-sm"
              role="group"
            ...
            {periodView === "monthly" ? (
              <Select value={String(month)} onValueChange={(v) => setMonth(Number(v))}>
                <SelectTrigger
                  className="h-10 w-[140px] rounded-xl border-border bg-card"
            ...
            <Select value={String(year)} onValueChange={(v) => setYear(Number(v))}>
              <SelectTrigger
                className="h-10 w-[100px] rounded-xl border-border bg-card"
          ...
          <AnimatePresence mode="wait">
            ...
            >
              ...
              <div className="lg:col-span-3">
                <div className="flex h-full min-h-[380px] flex-col rounded-2xl border border-border/80 bg-card p-5 shadow-sm">
