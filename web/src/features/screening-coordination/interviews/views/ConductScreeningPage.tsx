...
export default function ConductScreeningPage() {
  ...
  const handleWhatsAppShare = () => {
    if (!interview.meetingLink || !candidate) return;
    
    // Extract mobile number
    const mobileNumber = candidate.mobileNumber || candidate.phone || "";
    if (!mobileNumber) {
      toast.error("Candidate phone number not found");
      return;
    }

    // Clean number (remove non-digits, but keep + if lead)
    const cleanNumber = mobileNumber.replace(/[^\d+]/g, "");
    const message = `Hello ${candidate.firstName}, here is your meeting link for the screening of ${roleNeeded?.designation || "the role"}: ${interview.meetingLink}`;
    
    const whatsappUrl = `https://wa.me/${cleanNumber.startsWith('+') ? cleanNumber.slice(1) : cleanNumber}?text=${encodeURIComponent(message)}`;
    ...
    <div className="flex items-center justify-between w-full">
      ...
      <div className="flex items-center gap-4">
        <Button
          ...
          onClick={() => setEditInterviewOpen(true)}
        >
            ...
            {interview.meetingLink ? (
              <>
                <a
                  ...
                  className="text-sm font-medium text-indigo-600 hover:underline hover:text-indigo-700 break-all transition-colors truncate block"
                >
                  Join Meeting
                </a>
                <Button
                  ...
                  title="Share meeting link via WhatsApp"
                >
                  <svg
                    viewBox="0 0 24 24"
                    fill="currentColor"
  ...
  <CardContent className="p-6">
    {interview.template?.items && interview.template.items.length > 0 ? (() => {
      ...
      />
       ...
       <EditScreeningDialog
        open={editInterviewOpen}
        onOpenChange={setEditInterviewOpen}
        screeningId={routeId || ""}
      />
    </div>
  );
