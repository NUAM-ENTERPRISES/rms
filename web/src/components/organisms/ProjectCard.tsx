...
export default function ProjectCard({
  ...
  const totalPositions = project.rolesNeeded.reduce((sum, role) => sum + role.quantity, 0);
        ...
        <div className="flex items-start justify-between">
          ...
          <DropdownMenu>
            ...
            <DropdownMenuContent align="end">
              {canReadProjects && onView && (
                <DropdownMenuItem onClick={() => onView(project)}>
                  <Eye className="w-4 h-4 mr-2" />
                  View Details
                </DropdownMenuItem>
              )}
              {canManageProjects && onEdit && (
                <DropdownMenuItem onClick={() => onEdit(project)}>
                  <Edit className="w-4 h-4 mr-2" />
                  Edit Project
                </DropdownMenuItem>
              )}
              {canManageProjects && onDelete && (
                <DropdownMenuItem 
      ...
      <CardContent className="pt-0">
        <div className="space-y-3">
          ...
          <div className="flex items-center justify-between">
            <Badge 
              variant="outline" 
              className={cn("text-xs font-medium", getPriorityColor(project.priority))}
            >
