...
import { Check, ChevronsUpDown, Building2, Loader2, Plus, Users } from "lucide-react";
...
import { useGetClientsQuery, useGetClientQuery, type Client } from "@/features/clients";
...
export function ClientSelect({
  ...
  // Fetch full client details (with subClientLinks) when selected client has sub-clients
  const selectedSubClientCount = selectedClient?.subClientCount ?? 0;
  const { data: selectedClientDetail } = useGetClientQuery(value!, {
    skip: !value || selectedSubClientCount === 0,
  });
  ...
  return (
    <div className={cn("space-y-2", className)}>
          ...
          >
            {selectedClient ? (
              <div className="flex items-center gap-2 truncate">
                ...
                {selectedSubClientCount > 0 && (
                  <span className="inline-flex items-center gap-1 rounded-full bg-blue-50 border border-blue-200 px-1.5 py-0.5 text-xs text-blue-700 font-medium flex-shrink-0">
                    <Users className="h-3 w-3" />
                    {selectedSubClientCount} sub-client{selectedSubClientCount !== 1 ? "s" : ""}
                  </span>
                )}
              </div>
            ) : (
            ...
            <div className="overflow-y-auto">
              {isLoading && !clients.length ? (
                ...
                <div className="p-1">
                  ...
                  {clients.map((client) => {
                    const count = client.subClientCount ?? 0;
                    return (
                      <button
                        key={client.id}
                          ...
                          {count > 0 && (
                            <Tooltip>
                              <TooltipTrigger asChild>
                                <span
                                  className="inline-flex items-center gap-1 rounded-full bg-blue-50 border border-blue-200 px-1.5 py-0.5 text-xs text-blue-700 font-medium flex-shrink-0 cursor-default"
      ...
      {/* Sub-clients info panel shown after selection — uses full detail fetch */}
      {selectedSubClientCount > 0 && (() => {
        const subLinks = selectedClientDetail?.data?.subClientLinks ?? [];
        return (
          <div className="rounded-lg border border-blue-200 bg-blue-50 p-3 space-y-2">
            <p className="text-xs font-semibold text-blue-700 flex items-center gap-1.5">
              <Users className="h-3.5 w-3.5" />
              Linked sub-clients ({selectedSubClientCount})
            </p>
            {subLinks.length > 0 ? (
