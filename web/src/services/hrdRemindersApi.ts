...
export const hrdRemindersApi = baseApi.injectEndpoints({
  endpoints: (builder) => ({
    ...
    getHRDReminders: builder.query<HRDRemindersResponse, { dueOnly?: boolean } | void>({
      query: (arg) => {
        const dueOnly = (arg as any)?.dueOnly ? "?dueOnly=true" : "";
        return `/hrd-reminders/hrd-scheduler${dueOnly}`;
      },
